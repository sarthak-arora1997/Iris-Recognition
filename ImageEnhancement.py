...
# function: equalize the histogram of the image and return


    # equalize the histogram over the normalized image
...
# Note: As mentioned in the README file, many of the techniques mentioned in the Li Ma paper did not improve the accuracy of our pattern matching algorithm. The following code may be uncommented with the above code commented out in order to return an enhanced image as detailed in the paper.
...
# function: detect and remove glare and eyelashes from the normalized iris using thresholding absed off the histogram, then enhance the image using background illumination removal and histogram equalization over blocks of the normalized iris image
    ...
    # for our purposes, part 1 does not improve the quality of the enhancement, but rather results in block boundaries that act as false features
...
#     # loop to traverse the normalized image horizontally and vertically in 16x16 pixel blocks, obtaining and storing the means of the values in each section
